...
class Singlepost extends React.Component {
  ...
  render() {
    ...
    {
      var users = this.state.jsonuserdata;
    }

    return (
      <div>
        {" "}
        {!users || this.state.loading || !posts ? (
          <div> loading... </div>
        ) : (
          <div>
            <Link to={`/user/${posts[this.state.postid].userId}`}>
              <h2> {users[posts[this.state.postid].userId - 1].username} </h2>{" "}
            </Link>{" "}
            <h3> Post </h3> <p> {posts[this.state.postid].body} </p>{" "}
          </div>
        )}{" "}
      </div>
    );
